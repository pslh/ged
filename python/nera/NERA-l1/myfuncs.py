def get_replace_cost(sheet, building_type):
    from xlrd import open_workbook
    for y in range(1, sheet.nrows):
        b = str(sheet.cell(y,0).value)
        if b == building_type:
            return float(sheet.cell(y,1).value)
    return None


def sql_quote(x):
    import psycopg2.extensions 
    return psycopg2.extensions.adapt(x.encode('utf-8')).getquoted()
