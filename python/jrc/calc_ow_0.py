...
#
# This script was originally authored by Stefano Ferri of the JRC 
#
import sys
import os 
reload(sys)
...
# to avoid parameter request comment the 4 line below and type it into variables
...
def getPositionWallRoofCrosstab(wb,admin):
    textArray = (["Wall or building material"])
    if (admin == 'region'):
        found = False
        counter = 0
        dataset = None
            ...
            for row in range(s.nrows):
                ...
                for col in range(s.ncols):
                    try:
                        ...
                        if ((counter == 2) and (refVal in textArray)):
                            found = True
                            counter = counter + 1
...
def getArrays(s,regionStaringRow,roofMaterialRow):
    ...
    for row in range(regionStaringRow,s.nrows):
        ...
        for col in range(3,4):
            colMaterialName = "Unknown/missing"
...
def checkCountryExist(country,connection,mark):
    country = country.replace("'","''",10000000000)
    query = "SELECT id \
       FROM eqged.gadm_country where \"name\" = '"+country+"'";
    ...
    if (len(results) == 1):
        ...
        print "nooooo",country
        #exit()
        return None

def checkRegionExist(f,region,countryId,connection,mark,stampa):
    ...
    query = "SELECT id \
        FROM eqged.gadm_admin_1 \
        where gadm_country_id = " + str(countryId) + " and \
        ( \"name\" = '" + (region.replace("'","''"))+ "' OR varname = '" + (region.replace("'","''")) + "')"
    mark.execute(query)
    results = mark.fetchall()
    ...
    if (len(results) == 0):           
        ...
        if (len(results) == 0):
            if (stampa):
                ...
                ciccio = checkLevenshtein(f,region,countryId,connection,mark,stampa,string_compare)
                if (ciccio):
                    return ciccio
                else:
        ...
        if (len(results) > 1):
            if (stampa):
                print region + "  NO Inconsistency .. 0 in adm1 too many in adm2"
                if (f):
...
if (posWRC):
    ...
    roofMaterialRow = posWRC[1]+2
    regionStaringRow = posWRC[1]+2
    regionCol = 0
    wallMaterialCol = 2
