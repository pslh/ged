            ...
            for row in range(s.nrows):
                ...
                for col in range(s.ncols):
                    try:
                        ...
                        if ((counter == 3) and (refVal in textArray)):
                            found = True
                            counter = counter + 1
...
def getArrays(s,regionStaringRow,roofMaterialRow):
    ...
    for row in range(regionStaringRow,s.nrows):
        ...
        if (not (mainMatofWall == "Total")):
        
            for col in range(4,5):
                colMaterialName = 'Unknown/missing'#(s.cell(roofMaterialRow,col)).value
        ...
        if ((s.cell(row,3)).value == "Total"):
            valuex = (s.cell(row,4)).value
            totalsAreaRegion.append([area,region,valuex])
            print "total", s.cell(row,1), valuex
                    ###print totalsAreaRegion
                    #exit()
...
def getPagerValue(arraymsg,wall,roof,pagerArray,compare):
    ...
    if (x and y): 
        ...
        if (not(x)):
            ...
            arraymsg = addLogMsg(arraymsg,"can not find corrispondency in wall: " + str(wall))
            #f.write("can not find corrispondency in wall: " + wall + "\n")
            
        if (not(y)):
            ##print "Not Found ", roof
            #print "can not find corrispondency in wall: " + wall + "\n"
            arraymsg = addLogMsg(arraymsg,"can not find corrispondency in roof: " + str(roof))
...
if (posWRC):
    ...
    print roofMaterialRow,regionStaringRow
