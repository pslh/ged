...
#python calc_1_g2_s.py "Burkina Faso_StepI to StepV.xls" 1 55
import sys
import os 
reload(sys)
...
if (os.path.exists(localparamfile)):
    ...
    country_correspondences=param.readline()
else:
    ...
    country_correspondences=raw_input("Enter the country_correspondences")
...
def checkRegionExist(arraymsg,region,countryId,connection,mark,stampa,country_correspondences_array):
    ...
    ## search region in country_correspondences_array
    #print regionUTF8
    for items in country_correspondences_array:
        if (regionUTF8 == items[1]):
            regionUTF8 = items[0]
        ...
        if (len(results) == 0):
            if (stampa):
                ...
                if (ciccio):
                    ...
                    print regionUTF8 + " NO Not found in adm1,adm2"
                    exit()
                    arraymsg = addLogMsg(regionUTF8,region + " NO Not found in adm1,adm2")
                    return None,None
...
#read external corrspondance table
wbC = open_workbook(country_correspondences) 
...
print nameOfTheCountry
wbC = open_workbook(country_correspondences)

try:
    country_correspondences_s =  wbC.sheet_by_name(nameOfTheCountry)
except:
    print "cant find country_correspondences, i am looking for: " + nameOfTheCountry
    exit()


r=0
country_correspondences_array = []
for rownum in range(country_correspondences_s.nrows):
    #print country_correspondences_s.cell(rownum,r).value,country_correspondences_s.cell(rownum,r+1).value
    country_correspondences_array.append([country_correspondences_s.cell(rownum,r).value,country_correspondences_s.cell(rownum,r+1).value])
...
if (posWRC):
    ...
    for i in totalsAreaRegion:
        ...
        if not (region == latestregion):
            ###print region
            check = checkRegionExist(arraymsg,region,gadm_country_id,connection,mark,True,country_correspondences_array)
            if (check):
    ...
    for i in materialsArray: 
        ...
        if ((pagerValue) and (pagerValue <> "skip")):
            ...
            if region in foundRegion:
                check = checkRegionExist(arraymsg,region,gadm_country_id,connection,mark,False,country_correspondences_array)
                if (check):
                    i.append(pagerValue)
